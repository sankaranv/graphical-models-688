...
parents = np.array([None, None, [8], [1], [0, 1], [8], [0, 8], [8], [3, 4]])
...
for i in range(2):
    print(param_dict['BP|G'][:, i])
print('-------------')

for i in range(2):
    for j in range(2):
        print(param_dict['HD|BP,CH'][:, j, i])
print('-------------')

for i in range(2):
    for j in range(3):
        print(param_dict['HR|A,HD'][:, j, i])
print('-------------')


# Query the network

joint_ch = param_dict['CH|A,G'][:, 1, 1] * param_dict['HD|BP,CH'][0, 0, :]
joint_ch = joint_ch / np.sum(joint_ch)
print("Query 1: " + str(joint_ch))

joint_bp = param_dict['A'][1] * param_dict['BP|G'] * param_dict['G'] * \
    param_dict['CH|A,G'][1, 1, :] * \
    np.expand_dims(param_dict['HD|BP,CH'][0, :, 1], 1)
joint_bp = joint_bp / np.sum(joint_bp)
joint_bp = np.sum(joint_bp, axis=1)
print("Query 1: " + str(joint_bp))
...
def calc_heart_prob(model, datacase):
    ...
    joint_hd = param_dict['HD|BP,CH'][:, bp, ch] * param_dict['HR|A,HD'][hr, a, :] \
        * param_dict['ECG|HD'][ecg, :] * param_dict['EIA|HD'][eia, :] \
        * param_dict['CP|HD'][cp, :]
